...
class SentimentAnalyzer:
    ...
    def save_to_csv(self, filename):
        with open(filename, 'w', newline='') as csvfile:
            ...
            writer.writerow(["Message", "Sentiment Score", "Timestamp", "Exam Season", "Stress Reason"])
            for message, score, timestamp, is_exam, reason in zip(
                self.conversation, self.sentiment_scores, self.timestamps, self.exam_seasons, self.stress_reasons):
                writer.writerow([message, score, timestamp, is_exam, reason])
...
def extract_stress_reason(message):
    ...
    if "Exams"or "grades" or "project" or "deadlines" or "Balancing" or "studies" or "responsibilities" or "exams" in message:
...
# Generate the report
    try:
        ...
        sentiment_analyzer.save_to_csv('conversation_data.csv')
        print("Conversation data saved as 'conversation_data.csv'")
